...
const Testimonial = () => {
    ...
    const testimonials = [
        ...
        { name: "Lokesh, VIT Chennai", 
          location: "VIT Chennai", 
          image: assets.testimonial_image_4, 
          testimonial: "The booking process was seamless and the car was in excellent condition. Will use again!" 
        }
    ];
           ...
           <Title title="What Our Customers Say" subTitle="Discover why discerning travelers choose StayVenture for their luxury accommodations around the world."/>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-18">
                {testimonials.map((testimonial, index) => (
                    ...
                    key={index} className="bg-[#F5DEB3] p-6 rounded-xl shadow-lg hover:-translate-y-1 transition-all duration-500">

                        <div className="flex items-center gap-3">
                            <img className="w-12 h-12 rounded-full" src={testimonial.image} alt={testimonial.name} />
                            <div>
                                <p className="text-xl michroma-regular">{testimonial.name}</p>
                                <p className="text-gray-500 michroma-regular">{testimonial.location}</p>
                            </div>
                        </div>
