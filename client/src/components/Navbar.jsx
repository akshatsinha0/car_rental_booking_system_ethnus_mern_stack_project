...
const Navbar = ({ theme, setTheme }) => {
  ...
  return (
    ...
    className={`flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 text-text border-b border-borderColor relative transition-all ${location.pathname === "/" ? "bg-[#F5DEB3]" : "bg-[#F5DEB3]"}`}>
        ...
        <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor right-0 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 max-sm:p-4 transition-all duration-300 z-50 ${location.pathname === "/" ? "bg-[#F5DEB3]" : "bg-[#F5DEB3]"} ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
